...
public class DefaultSecurityManager implements SecurityManager {
	...
	public MainError validateSystemParameters(RopRequestContext context) {
		...
		// 0.检查timestamp,防止重放攻击
		if (context.getTimestamp() == null) {
			return MainErrors.getError(MainErrorType.MISSING_TIMESTAMP, context.getLocale(), context.getMethod(), context.getVersion(),
					SystemParameterNames.getTimestamp());
		}
		Date timestamp = null;
		try {
			timestamp = DateUtils.parseDate(context.getTimestamp(), DateUtils.SHOW_DATETIME_FORMAT);
		} catch (IllegalArgumentException e) {
			logger.error(e);
		}
		// 允许客户端请求时间误差为6分钟(暂时写死6分钟)
		Date now = new Date();
		now = org.apache.commons.lang3.time.DateUtils.addMinutes(now, -6);
		if (timestamp == null || now.after(timestamp)) {
			return MainErrors.getError(MainErrorType.INVALID_TIMESTAMP, context.getLocale(), context.getMethod(), context.getVersion(),
					SystemParameterNames.getTimestamp());
		}
		...
		AppKeyContext appKeyContext = appSecretManager.getAppKeyContext(context.getAppKey());
		context.setAppKeyContext(appKeyContext);
		if (!appSecretManager.isValidAppKey(appKeyContext)) {
		...
		if (context.getVersion() == null) {
			return MainErrors.getError(MainErrorType.MISSING_VERSION, context.getLocale(), context.getMethod(), SystemParameterNames.getVersion());
		} else {
			if (!ropContext.isValidVersion(context.getMethod(), context.getVersion())) {
				return MainErrors.getError(MainErrorType.UNSUPPORTED_VERSION, context.getLocale(), context.getMethod(), context.getVersion());
			}
		}
		...
		if (context.getServiceMethodDefinition().isObsoleted()) {
			return MainErrors.getError(MainErrorType.METHOD_OBSOLETED, context.getLocale(), context.getMethod(), context.getVersion());
		}
	...
	public MainError validateOther(RopRequestContext rrctx) {
		...
		// 1.判断应用/用户是否有权访问目标服务
		mainError = checkServiceAccessAllow(rrctx, rrctx.getAppKeyContext());
		if (mainError != null) {
			return mainError;
		}

		// 2.判断应用/会话/用户访问服务的次数或频度是否超限
		mainError = checkInvokeTimesLimit(rrctx, rrctx.getAppKeyContext());
		if (mainError != null) {
			return mainError;
		}
	...
	private MainError checkInvokeTimesLimit(RopRequestContext rrctx, AppKeyContext appKeyContext) {
		...
		} else if (invokeTimesController.isAppInvokeLimitExceed(rrctx.getAppKey(), appKeyContext)) {
	...
	private MainError validateHttpAction(RopRequestContext ropRequestContext) {
		...
		if (httpActions.length > 0) {
			...
			if (!isValid) {
				mainError = MainErrors.getError(MainErrorType.HTTP_ACTION_NOT_ALLOWED, ropRequestContext.getLocale(), ropRequestContext.getMethod(),
						ropRequestContext.getVersion(), ropRequestContext.getHttpAction());
			}
		}
		return mainError;
	}
	...
	private MainError checkServiceAccessAllow(RopRequestContext smc, AppKeyContext appKeyContext) {
		if (!getServiceAccessController().isAppGranted(smc.getAppKey(), smc.getMethod(), smc.getVersion(), appKeyContext)) {
		...
		if (context.isSignEnable()) {
			if (!context.getServiceMethodDefinition().isIgnoreSign()) {
				if (context.getSign() == null) {
					...
					String signSecret = context.getAppKeyContext().getAppSecret();
					if (signSecret == null) {
					...
					if (!signValue.equals(context.getSign())) {
						...
						return MainErrors.getError(MainErrorType.INVALID_SIGNATURE, context.getLocale(), context.getMethod(), context.getVersion());
					} else {
						return null;
					}
				}
	 ...
	 * @return
	 */
	private MainError getBusinessParameterMainError(List<ObjectError> allErrors, Locale locale, SubErrorType subErrorType, RopRequestContext context) {
